...
type NavigationProp = StackNavigationProp<RootStackParamList, "Home","Category">;
...
// Nhận promotions từ props
const Header = ({ promotions }: { promotions: Promotion[] }) => {
  const navigation = useNavigation<NavigationProp>();
  const isFocused = useIsFocused(); // Kiểm tra xem màn hình Home có đang focus hay không
  ...
  const [showBadge, setShowBadge] = useState(true); // State để kiểm soát hiển thị chấm đỏ
  ...
  // Khi màn hình không còn focus (rời khỏi Home), ẩn chấm đỏ
  useEffect(() => {
    if (!isFocused) {
      setShowBadge(false); // Ẩn chấm đỏ khi rời khỏi màn hình Home
    }
  }, [isFocused]);

  const handleSearchPress = () => {
    // Truyền promotions khi điều hướng sang Search
...
export const headerstyle = StyleSheet.create({
  ...
  badge: {
    ...
    right: -5,
    top: -5,
    backgroundColor: "#FF6347", // Đổi màu nếu muốn
    borderRadius: 10,
    width: 12, // Tăng kích thước
    height: 12,
